...
export default function TopBar() {
  ...
  const navItems = [
    { href: "/", label: "Accueil", icon: Home, active: true },
  ...
  return (
    <>
      <motion.div 
        className="sticky top-0 z-50 border-b border-white/10 bg-[#0a0a0a] shadow-lg shadow-black/50 px-4 md:px-8 py-2 text-[11px] text-gray-400 flex flex-wrap items-center justify-between uppercase tracking-wider"
            ...
            >
              ...
              >
                <item.icon className={`w-3.5 h-3.5 transition-transform duration-300 ${
                  item.active ? 'text-teal-400' : 'text-gray-500'
                }`} />
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div 
            className="md:hidden fixed inset-x-0 top-[57px] z-50 bg-[#0a0a0a] border-b border-white/10 shadow-lg shadow-black/50"
            initial={{ opacity: 0, y: -10 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
            transition={{ duration: 0.3 }}
            ...
            <div className="p-4 space-y-2">
              {navItems.map((item, index) => (
                ...
                >
                  ...
                  >
                    <item.icon className={`w-4 h-4 ${item.active ? 'text-teal-400' : ''}`} />
              ...
              <div className="border-t border-white/5 my-4 pt-4 space-y-3">
                ...
                <motion.div 
                  className="flex items-center gap-3 px-4 py-2 text-teal-400"
